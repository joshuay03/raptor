...
module Raptor
  class TestCLI < TestCase
    ...
    def test_chunk_data_timeout
      ...
      assert_equal 20, options(cli)[:connection][:chunk_data_timeout]
    end

    def test_write_timeout
      cli = CLI.new(["--write-timeout", "15"])

      assert_equal 15, options(cli)[:connection][:write_timeout]
    end

    def test_max_body_size
      cli = CLI.new(["--max-body-size", "1048576"])

      assert_equal 1048576, options(cli)[:connection][:max_body_size]
    end

    def test_body_spool_threshold
      cli = CLI.new(["--body-spool-threshold", "4096"])

      assert_equal 4096, options(cli)[:connection][:body_spool_threshold]
    end

    def test_http1_persistent_data_timeout
      cli = CLI.new(["--http1-persistent-data-timeout", "120"])

      assert_equal 120, options(cli)[:http1][:persistent_data_timeout]
    end

    def test_http1_max_keepalive_requests
      cli = CLI.new(["--http1-max-keepalive-requests", "50"])

      assert_equal 50, options(cli)[:http1][:max_keepalive_requests]
    end

    def test_http2_max_concurrent_streams
    ...
    def test_nested_options_are_independent_between_instances
      cli1 = CLI.new(["--first-data-timeout", "5"])
      cli2 = CLI.new([])

      assert_equal 5, options(cli1)[:connection][:first_data_timeout]
      assert_equal 30, options(cli2)[:connection][:first_data_timeout]
    end
